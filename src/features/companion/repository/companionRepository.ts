...
export async function createCompanion(
  ...
  await db.execute(
    ...
    INSERT OR REPLACE INTO companions (
      id, name, archetype, core_json, shape_json, mood_json, created_at
    )
    VALUES (?, ?, ?, ?, ?, ?, ?)
    `,
    [
...
export async function getAllCompanions(): Promise<CompanionRecord[]> {
  const db = await getDb();

  return db.select<CompanionRecord[]>(
    "SELECT * FROM companions ORDER BY created_at ASC"
  );
}